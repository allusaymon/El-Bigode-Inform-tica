...
def inserir_produtos(Nome,Quantidade,Preco):
    try:
        ...
        if Quantidade < 0:
            print("Erro: A quantidade não pode ser um número negativo!")
            messagebox.showerror("Erro de Validação", "A quantidade não pode ser negativa!")
            return
            
        if Preco < 0:
            print("Erro: O preço não pode ser um número negativo!")
            messagebox.showerror("Erro de Validação", "O preço não pode ser negativo!")
            return
